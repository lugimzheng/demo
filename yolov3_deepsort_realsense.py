...
import warnings
import numpy as np
import pyrealsense as rs # install by 'pip install pyrealsense'
...
class VideoTracker(object):
    def __init__(self, cfg, args, video_path, video_path_depth):
        self.cfg = cfg
        self.args = args
            ...
            if len(outputs) > 0:
                ...
                for bb_xyxy in bbox_xyxy:
                    ...
                    for idx in range(len(colors)):
                        ...
                        #            shift=0)
                        cv2.circle(f_color, (pose[idx, 0], pose[idx, 1]), 3, colors[idx], thickness=3, lineType=8,
                                   shift=0)
                    for idx in range(len(colors_skeleton)):
                        ...
                        f_color = cv2.line(f_color, (pose[pairs[idx][0], 0], pose[pairs[idx][0], 1]),
                                        (pose[pairs[idx][1], 0], pose[pairs[idx][1], 1]), colors_skeleton[idx], 3)
...
if __name__ == "__main__":
    ...
    with VideoTracker(cfg, args, video_path=args.VIDEO_PATH, video_path_depth=args.VIDEO_PATH_Depth) as vdo_trk:
        vdo_trk.run()
